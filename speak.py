...
while True:
    pyttsx3.speak("what  AYUSH  you      want : ")
    print("what you want:")
    cmd = input()
    ...
    if (run and (("chrome" in cmd) or ("browser" in cmd))):  # chrome
        ...
        pyttsx3.speak("open ning chrome")
        os.system("chrome")
    ...
    elif ((run or play) and (
            (("player" in cmd) and ("video player" not in cmd)) or ("mp3" in cmd))):  # window media player
        if (dont):
            continue
        os.system("wmplayer")
        ...
        if (dont):
            continue
        pyttsx3.speak("open ning notepad")
        os.system("notepad")
        ...
        if (dont):
            continue
        pyttsx3.speak("closingggggggggg")
        break

    else:
        pyttsx3.speak("Sorry , Please Try another")
